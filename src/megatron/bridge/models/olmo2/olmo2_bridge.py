...
@MegatronModelBridge.register_bridge(source=Olmo2ForCausalLM, target=GPTModel)
class OLMo2Bridge(MegatronModelBridge):
    """
    Megatron Bridge for OLMo2 Models.

    This bridge handles the conversion between HuggingFace Olmo2ForCausalLM
    and Megatron-Core GPTModel formats. OLMo2 is a decoder-only transformer
    from Allen Institute for AI.

    Example:
        >>> from megatron.bridge import AutoBridge
        >>> bridge = AutoBridge.from_hf_pretrained("allenai/OLMo2-7B")
        >>> provider = bridge.to_megatron_provider()
    """

    def provider_bridge(self, hf_pretrained: PreTrainedCausalLM) -> OLMo2ModelProvider:
